...
ssize_t v9fs_iov_vunmarshal_one(struct iovec *out_sg, int out_num,
                                size_t *offset, int bswap, char fmt,
                                va_list *ap)
{
    ssize_t copied = 0;
                            ...
                            int bswap, const char *fmt, va_list ap)
{
    ...
    for (i = 0; fmt[i]; i++) {
        ...
        copied = v9fs_iov_vunmarshal_one(out_sg, out_num, &offset, bswap,
                                         fmt[i], &aq);
        if (copied < 0) {
            return copied;
        }
                          ...
                          int bswap, const char *fmt, va_list ap)
{
    ...
    for (i = 0; fmt[i]; i++) {
        ...
        copied = v9fs_iov_vmarshal_one(in_sg, in_num, &offset, bswap, fmt[i],
                                       &aq);
        if (copied < 0) {
            return copied;
        }
